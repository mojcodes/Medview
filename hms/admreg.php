		...
		if (empty($token)) {
			...
			exit();		
				...
				if ($_SESSION['captcha_token'] == $token) {
					...
					$query=mysqli_query($con,"insert into admini (username,email,password) values('$uname','$email','$password')");
					//$query=mysqli_query($con,"insert into admin(username,password,updationDate) values('$fname','$password','$da')");
					if($query)
					{
